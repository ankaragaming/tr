...
document.addEventListener("DOMContentLoaded", function() {
  ...
  function processCampaignElements() {
      ...
      campaignElements.forEach(element => {
          ...
          for (const [type, config] of Object.entries(campaignTypes)) {
              if (config.variants.some(variant => content.includes(variant))) {
  ...
  document.querySelectorAll('.item-content.prime, .item-content.gameon, .item-content.bi-dunya, .item-content.selfy')
      .forEach(element => {
...
function MobileDescriptionAdd(){
  ...
  if(bireyselLocation && mobilLocation){
    ...
    const mobilDetailPageButton = mobilDetailPageDiv?.querySelector('.detail-text-btns-wrap');
    ...
    if(location.href.includes('faturasiz-paketler') || location.href.includes('faturasiz-tarifeler')){
      ...
      mobilDetailPagePriceDesc.innerHTML = "Online satış kanalından numara taşıma ve yeni hat tesisi ile faturalı tarifelere yapılan başvuruların aktivasyon işlemlerinde on taksit şeklinde <strong> 120 TL Dijital Kurye </strong> ücreti alınmaktadır.";
    }
